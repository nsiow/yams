...
// Explain returns the [Trace] formatted as a barebones array of messages
...
// Trace returns the [Trace] formatted as a barebones human-readable string
...
// Print returns the [Print] formatted as a full human-readable string
func (t *Trace) Print() string {
	p := Printer{}
	t.Walk(&p)
	return p.Print()
}
...
func (p *Printer) Annotate(fr *Frame) string {
	if fr.Attributes["allow"] == "true" {
		return "(allow) "
	}

	if fr.Attributes["deny"] == "true" {
		return "(deny) "
	}
...
// Explainer is a specialized [Walker] which walks the [Trace] and attempts to emit only the most
// useful/relevant messages for concisely explaining the evaluation
type Explainer struct {
	messages []string
}

func (s *Explainer) FrameStart(f *Frame) {
	_ = f // required for test coverage
}

func (s *Explainer) FrameEnd(f *Frame) {
	_ = f // required for test coverage
}

func (p *Explainer) Add(s string) {
	p.messages = append(p.messages, s)
}

func (p *Explainer) Message(fr *Frame, msg string) {
	if fr.Attributes["allow"] == "true" || fr.Attributes["deny"] == "true" {
		p.Add(
			msg,
		)
	}
